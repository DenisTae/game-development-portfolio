...

{
    ...
    target:6,
    type:"tap",
    tip:"Cada gota cuenta. Cierra los grifos cuando no los estés usando."
},

{
    ...
    target:9,
    type:"trash",
    tip:"Cuida los ríos y evita tirar basura. El agua limpia es responsabilidad de todos."
},

{
    ...
    target:12,
    type:"trash",
    tip:"Recoger residuos ayuda a mantener nuestras fuentes de agua limpias."
},

{
    ...
    target:16,
    type:"plastic",
    tip:"Evita los plásticos. Menos contaminación significa océanos más saludables."
},

{
    ...
    target:22,
    type:"water",
    tip:"¡El planeta depende de nosotros! Usa el agua de forma responsable todos los días."
}
...
let bestScore =
Number(localStorage.getItem("heroWaterScore")) || 0;

let bestCombo =
Number(localStorage.getItem("heroWaterCombo")) || 0;

let bestLevel =
Number(localStorage.getItem("heroWaterLevel")) || 0;
...
function update(dt){
    ...
    Math.max(
        45,
        Math.min(world.w-45,player.x)
    );

    player.y =
    Math.max(
        120,
        Math.min(world.h-45,player.y)
    );


    player.inv =
    Math.max(0,player.inv-dt);

    player.shield =
    Math.max(0,player.shield-dt);

    player.magnet =
    Math.max(0,player.magnet-dt);
...
function spawnObject(){
    ...
    const types =
    level === 1
    ? ["tap","tap","tap","water","trash","danger","power"]

    : level === 2
    ? ["trash","trash","water","danger","power"]

    : level === 3
    ? ["trash","trash","plastic","water","danger","power"]

    : level === 4
    ? ["plastic","plastic","trash","water","danger","power"]

    : ["water","water","trash","plastic","tap","danger","power"];
...
function gameMessage(
    ...
    $("game").parentElement.appendChild(box);
