const dialogue=document.getElementById("dialogue"),speaker=document.getElementById("speaker"),next=document.getElementById("next"),choices=document.getElementById("choices"),phone=document.getElementById("phone"),phoneScreen=document.getElementById("phone-screen"),emotion=document.getElementById("emotion"),perspective=document.getElementById("perspective"),locationText=document.getElementById("location"),transition=document.getElementById("transition"),transitionTitle=document.getElementById("transition-title"),transitionSub=document.getElementById("transition-sub"),alex=document.getElementById("alex"),mateo=document.getElementById("mateo"),dani=document.getElementById("dani");

let scene=[],position=0,typing=false,typingTimer=null,nextAction=null,phoneTimer=null,emotionTimer=null;

const state={act:1,alexIsolation:0,alexAnxiety:0,support:0,mateoEmpathy:0,mateoHarm:0,defended:false,message:false};
...
[alex,mateo,dani].forEach(c=>c&&c.classList.remove("active"));
...
phoneScreen.innerHTML=`<div class="phone-post"><b>Tu publicación</b><div class="phone-comment">Mateo: ¿De verdad vas a subir eso?</div><div class="phone-comment">Lucas: JAJA.</div><div class="phone-comment">Mateo: Mejor bórralo.</div></div>`;
phone.classList.add("show")
}

function showPhoneNotification(){
if(!phone||!phoneScreen)return;
phoneScreen.innerHTML=`<div class="phone-notification">♡ Nueva notificación</div><div class="phone-post">Tienes nuevos comentarios.</div>`;
phone.classList.add("show")
}

function showPhoneMessage(){
if(!phone||!phoneScreen)return;
phoneScreen.innerHTML=`<div class="phone-post"><b>Mensajes</b><div class="phone-message">Mateo: ¿Estás bien?</div></div>`;
phone.classList.add("show")
}
...
emotionTimer=setTimeout(()=>emotion.classList.remove("show"),2500)
}
...
phoneTimer=setTimeout(()=>showPhoneNotification(),1500)
}
...
if(phoneTimer)clearTimeout(phoneTimer);
phoneTimer=setTimeout(()=>showPhonePost(),1200)
}
...
if(phoneScreen)phoneScreen.innerHTML=`<div class="phone-notification">♡ Nuevo comentario</div><div class="phone-post">Lucas: ¿Todavía sigues con eso?</div>`;
...
if(phoneTimer)clearTimeout(phoneTimer);
phoneTimer=setTimeout(()=>showPhonePost(),1000)
}
...
if(phoneTimer)clearTimeout(phoneTimer);
phoneTimer=setTimeout(()=>showPhoneMessage(),700)
}
},
{
text:"Guardar el teléfono",
